...
class WorkflowConfigurationTest(unittest.TestCase):
    ...
    def test_cafe24_queue_has_order_number_search_form(self):
        queue_source = (APP_ROOT / "static" / "admin" / "cafe24-queue-ui.js").read_text()
        actions_source = (APP_ROOT / "static" / "admin" / "cafe24.js").read_text()
        sections_source = (APP_ROOT / "static" / "admin" / "sections.js").read_text()

        self.assertIn("data-admin-cafe24-search-form", queue_source)
        self.assertIn("주문번호로 검색", queue_source)
        self.assertIn("data-admin-cafe24-clear-search", queue_source)
        self.assertIn('form.matches("[data-admin-cafe24-search-form]")', actions_source)
        self.assertIn("data-admin-cafe24-clear-search", actions_source)
        self.assertIn("상세/수동 처리 열기", sections_source)
        self.assertIn("상품 조회/진단/연동 설정", sections_source)
