...
def _order_item_summary(order_items: list[Dict[str, Any]]) -> Dict[str, int]:
    return {
        ...
        "autoDispatchExcludedCount": sum(1 for item in order_items if item["standardStatus"] == "auto_dispatch_excluded"),
...
def _manual_workflow_payload(
    ...
    *,
    ...
    auto_dispatch_excluded_count = _scalar_count(
        conn.execute(
            f"""
            SELECT COUNT(*) AS count
            {active_item_join}
    ...
    return {
        ...
        "autoDispatchExcludedCount": auto_dispatch_excluded_count,
