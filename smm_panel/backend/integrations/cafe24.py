...
CAFE24_AUTO_DISPATCH_EXCLUDED_PRODUCT_NAME_TOKENS = ("개인결제",)
CAFE24_AUTO_DISPATCH_EXCLUDED_PRODUCT_NOS_ENV_KEYS = (
    "SMM_PANEL_CAFE24_AUTO_DISPATCH_EXCLUDED_PRODUCT_NOS",
    "CAFE24_AUTO_DISPATCH_EXCLUDED_PRODUCT_NOS",
)
CAFE24_STANDARD_STATUSES = {
    ...
    "failed",
    "cancelled",
    CAFE24_AUTO_DISPATCH_EXCLUDED_STATUS,
    *CAFE24_MANUAL_INPUT_REQUIRED_STATUSES,
...
def cafe24_order_item_summary_payload(row: Dict[str, Any] | None) -> Dict[str, int]:
    ...
    return {
        ...
        "autoDispatchExcludedCount": int(row_map.get("auto_dispatch_excluded_count") or 0),
...
def cafe24_auto_dispatch_excluded_product_nos() -> set[str]:
    values: set[str] = set()
    for key in CAFE24_AUTO_DISPATCH_EXCLUDED_PRODUCT_NOS_ENV_KEYS:
...
def cafe24_auto_dispatch_exclusion_reason(
    ...
    *,
    ...
    product_no = str(identity.get("productNo") or cafe24_payload_value(item_payload, ("product_no", "productNo"))).strip()
    if product_no and product_no in cafe24_auto_dispatch_excluded_product_nos():
        return f"Cafe24 상품번호 {product_no}은 자동발주 제외 정책에 포함되어 있습니다. 수동 처리 후 상태만 관리하세요."
    ...
    product = item_payload.get("product") if isinstance(item_payload.get("product"), dict) else {}
    if product:
        text_candidates.append(
            cafe24_payload_value(product, ("product_name", "productName", "name"))
        )
    combined = " ".join(text for text in text_candidates if text)
    if any(token in combined for token in CAFE24_AUTO_DISPATCH_EXCLUDED_PRODUCT_NAME_TOKENS):
        return "개인결제 상품은 공급사 자동발주 대상이 아닙니다. 수동 처리 후 상태만 관리하세요."
    return ""
