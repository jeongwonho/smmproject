...
function renderLegacyOperationalReadiness(audit, escapeHtml) {
  ...
  const cards = [
    ...
    },
    {
      ...
      detail: `수동 보정 ${manualInputCount}건 · 검토 ${reviewCount}건`,
    ...
    <div class="admin-panel">
      ...
      <div class="admin-mapping-preview">
        ...
        <article class="admin-mini-card ${Number(workflow.manualInputRequiredCount || 0) ? "is-risk" : ""}">
          ...
          <p>공급사 ready ${escapeHtml(String(workflow.supplierReadyCount || 0))}곳 · 매핑 필요 ${escapeHtml(String(workflow.mappingRequiredCount || 0))}건</p>
        </article>
        <article class="admin-mini-card">
