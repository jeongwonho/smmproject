...
실제 판매 지향형 SMM 쇼핑몰과 운영 콘솔입니다.  
`홈 → 상품목록 → 상품상세 → 로그인/회원가입 → 충전 → 주문내역 → 마이` UX와 플랫폼/카테고리/상품/주문/회원/동의 이력 DB 구조를 분리해 구현했습니다.
...
구조 변경과 신규 기능 추가 시에는 `docs/architecture-guidelines.md` 를 기준으로 작업합니다. 빌드 단계에서 `scripts/architecture-check.mjs` 가 실행되어 고객/관리자 코드 경계와 파일 크기 예산을 검사합니다.
...
- `SMM_PANEL_CAFE24_CLIENT_ID`: Cafe24 OAuth 앱 Client ID
- `SMM_PANEL_CAFE24_CLIENT_SECRET`: Cafe24 OAuth 앱 Client Secret
- `SMM_PANEL_CAFE24_REDIRECT_URI`: 선택값. Cafe24 앱에 등록한 콜백 주소. 미설정 시 현재 도메인의 `/api/admin/cafe24/oauth/callback` 사용. Cafe24 Developers에 등록한 Redirect URL과 1글자까지 동일해야 하며, 끝 `/` 포함 여부도 맞춰야 합니다.
...
## Cafe24 Gmail 2차 검증 자동화

`Cafe24 Gmail Order Witness` GitHub Actions workflow는 최근 Cafe24 주문 안내 메일을 읽고, 메일에서 추출한 주문번호를 `/api/cron/cafe24/email-order-witness` 로 전달합니다. 서버는 이미 DB에 있는 주문은 건너뛰고, 없는 주문번호만 Cafe24 단건 재수집으로 보정합니다.

GitHub Repository Secrets:

- `CAFE24_GMAIL_CLIENT_ID`: Gmail API OAuth Client ID
- `CAFE24_GMAIL_CLIENT_SECRET`: Gmail API OAuth Client Secret
- `CAFE24_GMAIL_REFRESH_TOKEN`: `https://www.googleapis.com/auth/gmail.readonly` 범위 refresh token
- `CRON_SECRET`: Vercel `CRON_SECRET` 과 같은 값. 없으면 GitHub OIDC 인증을 사용합니다.

선택 GitHub Repository Variables:

- `CAFE24_GMAIL_QUERY`: 기본값 `from:no-reply@cafe24shop.com newer_than:2d`
- `CAFE24_GMAIL_MAX_MESSAGES`: 기본값 `20`

Gmail 자격증명이 없으면 workflow는 `skipped` 로 종료하고 5분 체인을 만들지 않습니다. 자격증명을 넣으면 30분마다 부트스트랩된 뒤 `workflow_dispatch` 체인으로 5분 간격 확인을 이어갑니다. Gmail 본문 전체는 서버로 보내지 않고 주문번호와 Gmail message id만 전송합니다.
...
- `SMM_PANEL_CAFE24_CLIENT_ID`: Cafe24 Admin API OAuth Client ID
- `SMM_PANEL_CAFE24_CLIENT_SECRET`: Cafe24 Admin API OAuth Client Secret
- `SMM_PANEL_CAFE24_REDIRECT_URI`: Cafe24 OAuth 콜백 주소. 예: `https://your-domain.example/api/admin/cafe24/oauth/callback`. Cafe24 Developers의 Redirect URL과 정확히 같은 값을 넣어야 합니다.
